...
#ifdef _USE_EXHW_MCP2515

#define _PIN_DEF_MCP2515_CS spi1_cs_Pin
#define _PORT_DEF_MCP2515_CS spi1_cs_GPIO_Port
#define _GPIO_PIN_STATE_ON  GPIO_PIN_RESET
#define _GPIO_PIN_STATE_OFF  GPIO_PIN_SET
...
void mcp2515csPinWrite(uint8_t ch, bool is_levelhigh)
{
  ...
  if (ch == _DEF_CAN1)
  {
    ...
    HAL_GPIO_WritePin(_PORT_DEF_MCP2515_CS, _PIN_DEF_MCP2515_CS, is_levelhigh?_GPIO_PIN_STATE_OFF:_GPIO_PIN_STATE_ON);
  }
}
...
bool mcp2515Init(void)
{
  ...
  if (ret == false)
  {
    logPrintf("mcp2515Init - spiBegin Fail\n"); 
    return ret;
  }
...
void mcp2515Info(uint8_t ch)
{
  ...
  if (reg_bits == 0x02)
    cliPrintf("Loopback");
  if (reg_bits == 0x03)
    cliPrintf("Listen-Only");
  if (reg_bits == 0x04)
    cliPrintf("Configuration");
  cliPrintf("\n");
