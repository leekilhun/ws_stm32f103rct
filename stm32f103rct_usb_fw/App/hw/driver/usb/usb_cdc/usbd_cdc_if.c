...
static uint8_t q_rx_buf[APP_RX_DATA_SIZE];
static uint8_t q_tx_buf[APP_TX_DATA_SIZE];
...
{
  is_opened = false;
  qbufferCreate(&q_rx, q_rx_buf, APP_RX_DATA_SIZE);
  qbufferCreate(&q_tx, q_tx_buf, APP_TX_DATA_SIZE);
