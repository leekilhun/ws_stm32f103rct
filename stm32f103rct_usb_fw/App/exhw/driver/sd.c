...
#define _PIN_GPIO_SDCARD_DETECT sd_det_Pin
#define _PORT_GPIO_SDCARD_DETECT sd_det_GPIO_Port
#define _PIN_GPIO_SDCARD_CS sdcard_cs_Pin
#define _PORT_GPIO_SDCARD_CS sdcard_cs_GPIO_Port

#define _GPIO_IN_STATE_ON GPIO_PIN_RESET
#define _GPIO_IN_STATE_OFF GPIO_PIN_SET
#define _GPIO_OUT_STATE_ON GPIO_PIN_RESET
#define _GPIO_OUT_STATE_OFF GPIO_PIN_SET
...
static uint8_t spi_ch = _SPI_SDCARD;
...
static void sdSpiCS(bool is_levelhigh);
...
bool sdInit(void)
{
  ...
  if (spiIsBegin(spi_ch) != true)
  {
    ...
    {
      LOG_PRINT("[x] sdInit spiBegin Fail");
      return false;
    }
    else
    {
      LOG_PRINT("[O] sdInit spiBegin");
    }
  }
  else
  {
  ...
  if (sdIsDetected())
  {
    LOG_PRINT("[O] sdInit sdIsDetected");
    is_detected = true;
  }
  else
  {
    LOG_PRINT("[x] sdInit sdIsDetected");
  }
  ...
  if (is_detected == true)
  {
    ...
    {
      ret = true;
      LOG_PRINT("[O] sdInit sdSpiInitialize");
    }
  }
...
{
  if (HAL_GPIO_ReadPin(_PORT_GPIO_SDCARD_DETECT, _PIN_GPIO_SDCARD_DETECT) == _GPIO_IN_STATE_ON)
  {
    is_detected = true;
  }
  else
...
void sdSpiCS(bool is_levelhigh)
{
  // gpioPinWrite(_PIN_GPIO_SDCARD_CS, enable);
  HAL_GPIO_WritePin(_PORT_GPIO_SDCARD_CS, _PIN_GPIO_SDCARD_CS, is_levelhigh ? _GPIO_OUT_STATE_OFF : _GPIO_OUT_STATE_ON);
}
...
void sdSpiPowerOn(void)
{
  ...
  for (int i = 0; i < 10; i++)
  {
    if (sdSpiTxByte(0xFF) == false)
    {
      LOG_PRINT("sdSpiTxByte Fail!  index : %d", i);
    }
  }
  ...
  for (int i = 0; i < 6; i++)
  {
    if (sdSpiTxByte(cmd_arg[i]) == false)
    {
      LOG_PRINT("sdSpiTxByte(cmd_arg) Fail!  index : %d", i);
    }
  }

  LOG_PRINT("sdSpiTxByte(cmd_arg) Success!");

  /* 응답 대기 */
  uint32_t pre_time = millis();
  while (((rx_data = sdSpiRxByte()) != 0x01) && Count)
  {
    if (millis() - pre_time >= 2 * 1000)
    {
      LOG_PRINT("timeout - sdSpiRxByte() == %d", rx_data);
      break;
    }
    Count--;
  }

  LOG_PRINT("sdSpiRxByte() == %d", rx_data);
...
bool sdSpiInitialize(void)
{
  ...
  spiSetClockDivider(spi_ch, SPI_CLOCK_DIV_1);
  LOG_PRINT("spiSetClockDivider");
  /* SD카드 Power On */
  sdSpiPowerOn();
  LOG_PRINT("sdSpiPowerOn");
  ...
  if (sdSpiSendCmd(CMD0, 0) == 1)
  {
    ...
    {
      LOG_PRINT("sdSpiSendCmd Result == 1");
      /* SDC Ver2+ */
      for (n = 0; n < 4; n++)
      {
        ocr[n] = sdSpiRxByte();
  ...
  sdSpiCS(_DEF_HIGH);
  uint8_t temp = sdSpiRxByte(); /* Idle 상태 전환 (Release DO) */
  LOG_PRINT("sdSpiRxByte() == %d", temp);
  ...
  spiSetClockDivider(spi_ch, SPI_CLOCK_DIV_2);
  LOG_PRINT("sdSpiPowerOff - spiSetClockDivider");
...
void cliSd(cli_args_t *args)
{
  ...
  else if (args->argc == 2 && args->isStr(0, "cspin") == true)
  {
    uint8_t enable;

    enable = args->getData(1);
  ...
  {
    cliPrintf("sd info\n");
    cliPrintf("sd cspin [1:0]<level low:high>\n");
